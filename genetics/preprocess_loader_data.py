...
##data arguments
def preprocess_loader_data(keep_prs=False, duplicate_rows="last", prs_from_loader=None, saveName=None,
    ...
    if use_prsLoader:
        try:
            loaded_prs = PRSLoader().get_data().df.copy()[
                prs_from_loader]  # load prs with loader, assume that prs_from_loader is not none
        except KeyError:
            return None
    else:
        loaded_prs = \
        pd.read_csv("/net/mraid08/export/jasmine/zach/scores/score_results/SOMAscan/scores_all_raw.csv").set_index(
            "RegistrationCode")[prs_from_loader]  # load prs with loader, assume that prs_from_loader is not none
