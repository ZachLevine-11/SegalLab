...
class clusterTree:
    def __init__(self, by = "PRS", redo_original = False, withSave = False):
        ...
        self.stackmat = self.stackmat[pd.Series(self.stackmat.columns[(self.stackmat < 0.005).any()].values.tolist()).drop_duplicates()]
        ##keep only phenotypes with at least one significnat interaction
        self.stackmat = self.stackmat.loc[pd.Series(self.stackmat.index[(self.stackmat < 0.005).any(1)].values.tolist()).drop_duplicates(),]
        if by == "PRS":
    ...
    def testClusters(self,nsim = 1000, how = "sigclust2_tree"):
        if how == "sigclust2_tree":
            ...
            for i in range(nsim):
                generatedsim = normal(loc = 0, size = self.stackmat.shape[0]*self.stackmat.shape[1], scale =np.sqrt(eigval_sim_stacked))
                ...
                clusterindices_finalmerge_sim[i] = self.generateDistIndices(random_tree, override = True, rawcluster = random_cluster)
                print("finished simulation: " + str(i))
            for node_key in self.all_nodes:
                self.all_nodes[node_key].pval = norm.cdf(self.ClusterIndices[node_key], np.mean(clusterindices_finalmerge_sim), np.std(clusterindices_finalmerge_sim))
            ...
            with qp(jobname="z_genetics", max_r=10, max_u=10, _suppress_handlers_warning=True) as q:
                ...
                fundict = {}
                while i < nsim:
                    fundict[i] = q.method(randomCluster, (self.original_stackmat, i, True))  ##test can be "t" for t test or "r" for regression))
                    i += 1
...
#            os.chdir(gencove_logs_path)
            with qp(jobname="perms", max_r=10, max_u=10, _suppress_handlers_warning=True) as q:
                ...
                fundict = {}
                while i < nsim:
                    fundict[i] = q.method(randomCluster, (self.original_stackmat, i, False))  ##test can be "t" for t test or "r" for regression))
                    i += 1
                fundict = {k: q.waitforresult(v) for k, v in fundict.items()}
            res = reduce(lambda x,y: x.add(y, fill_value = 0), fundict.values())
            self.pvals_df = 1 - res/(nsim+1) ##equivalent of adding a small epsilon to the results so we never get a p value of zero
