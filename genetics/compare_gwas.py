...
def compareGwases(tenk_gwas_name, ukbb_gwas_name,
                  mainpath="/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/ldsc/", bothTenK=False):
...
def get_ukbb_gwas_loc(prs_name):
    ...
    return "/net/mraid08/export/genie/10K/genetics/PRSice/SummaryStatistics/Nealelab/v3/TransformedData/" + \
           prs_name.split("pvalue_")[-1] + ".gwas.imputed_v3.both_sexes.tsv"


##From HDL, exported using R
def read_snp_dictionary():
    return pd.read_csv("/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/ldsc/snp_dictionary.csv").drop(
...
def prepare_tenk_gwas(tenk_fname, mainpath="/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/ldsc/"):
    ...
    try:
        subprocess.call(
            ["~/PycharmProjects/genetics/prepare_tenk_gwas.sh" + " " + first_arg + " " + second_arg + " " + third_arg],
            shell=True)
    except Exception:
        return -1
    return 0
...
def munge_tenk_batched(tenk_batch):
    ...
    broken_tenk_phenos = [f for f in
                          os.listdir("/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/ldsc/broken_tenk_phenos/")
                          if isfile(
            join("/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/ldsc/broken_tenk_phenos/", f))]
...
def compare_gwases_batched(tenk_fnames, ukbb_fnames, exclude_broken_tenk_phenos=False):
    if exclude_broken_tenk_phenos:
        broken_tenk_phenos = [f for f in os.listdir(
            "/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/ldsc/broken_tenk_phenos/") if isfile(
            join("/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/ldsc/broken_tenk_phenos/", f))]
    else:
        broken_tenk_phenos = []
...
def compute_all_cross_corr(already_munged_all=True, exclude_broken_tenk_phenos=False, batch_width=10,
                           containing_dirs=["/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_results/",
                                            "/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/metab/gwas_results_metab/",
                                            "/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/microbiome/gwas_results_mb/"]):
    if exclude_broken_tenk_phenos:
        broken_tenk_phenos = [f for f in os.listdir(
            "/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/ldsc/broken_tenk_phenos/") if isfile(
            join("/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/ldsc/broken_tenk_phenos/", f))]
    ...
    with qp(jobname="ldsc") as q:
        ...
        if not already_munged_all:
            ...
            all_tenk_fnames = [x for x in all_tenk_fnames if
                               x not in broken_tenk_phenos and x != "/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_results/batch0.prs.glm.linear" and "clumpheader" not in x]
