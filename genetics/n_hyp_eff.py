...
def get_n_hyp_eff(loader_dict):
    ...
    for baseSaveName in loader_dict.keys():
        fundict[baseSaveName] = pd.read_csv("/net/mraid08/export/jafar/Zach/" + baseSaveName)
        print("combined with " + str(baseSaveName))
    all_loaders = pd.concat(fundict.values(), axis=1, join="inner") ##all loaders joined by index, for each person this has all their loader data
