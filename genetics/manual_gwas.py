...
def manual_assoc_test(boxplot, SNPdf, pheno):
    covars = pd.read_csv("/net/mraid08/export/jasmine/zach/height_gwas/covariates_with_age_gender.txt", sep="\t").drop("FID", axis=1)
    ...
    fancydf = SNPdf.reset_index("variant", drop = True).merge(pheno, left_index = True, right_index = True, how = "inner").dropna(axis = 0).merge(covars, left_index=True, right_index=True, how="inner")
    fancydf = fancydf[["genotype", pheno.name, "PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10", "age", "gender"]]
    ...
    try:
        ...
        if boxplot:
            ...
            plt.title("r^2 with =" + str(model.rsquared)[0:5] + ", r^2 without = " + str(ols(formula_without,fancydf).fit().rsquared)[0:5])
            plt.show()
...
def manual_associate_top_k_hits_compare(boxplot = True, var = 'pregnancy_end_reason_[JA62]', loader = ChildrenLoader, plink_data = None, gwas_results_renamed_dir = "/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_results_renamed/"):
    manual_res = {}
    plink_res = {}
    ...
    if len(read_in_gwas_res.P) != 0:  # ignore gwases with no significant associations and do FDR
        ...
        for rsid in top_plink_results.ID:
            snp_df = plink_data.loc[:, plink_data.snp == rsid].to_dataframe().drop("gender", axis=1)  # the #gender column is meaningless here
            manual_res[rsid] = manual_assoc_test(boxplot, snp_df, phenotype)
            plink_res[rsid] = top_plink_results.loc[top_plink_results.ID == rsid,"P"].values[0] ##drop the row index and the array dimension
