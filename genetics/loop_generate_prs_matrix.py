...
def loop_generate_prs_matrix(test = "m", duplicate_rows = "mean", saveName = None, tailsTest = "rightLeft", random_shuffle_prsLoader = False, use_prsLoader = True):
    fundict = {}
    ...
    for prs_id in range(len(prses)):
        ...
        fundict[prs_id] = q_loop(test, duplicate_rows, prses[prs_id], saveName, tailsTest, random_shuffle_prsLoader, use_prsLoader, prs_id)  ##test can be "t" for t test or "r" for regression))
