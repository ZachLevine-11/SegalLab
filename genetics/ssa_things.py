...
def find_lset(W, Akay, x, M = None, eps = 1e-3, how = "manual"):
    if how == "manual":
        ...
        ##based on https://github.com/kieferk/pymssa/blob/master/pymssa/mssa.py
            ...
            maxKeep = np.where(pctl > W)[0][0] ##find first place where noise eigenvalue real > actual eigenvalue real to truncate
...
def calibrate_SSA(x, Mset = list(range(1, 200, 5)), dt = 1):
    ...
    for M in Mset:
        ...
        W, Akay  = SSA(origx, M = M, reconstruct = False) ##ssa already centers and normalizes the data so just feed it the original time series
