...
def correct_all_loaders(loaders = None, correct_beforehand = False, plink_data = None, most_frequent_val_max_freq = 0.95, min_subject_threshold = 3000):
    for loader in loaders:
        operative_loader = read_loader_in(loader)
        operative_loader = pre_filter(operative_loader, plink_data = plink_data, most_frequent_val_max_freq = most_frequent_val_max_freq, min_subject_threshold = min_subject_threshold)
        justname = str(loader).split(".")[2] + ".csv"
        saveName = corrected_loader_save_path + justname
        if correct_beforehand:
            correct_all_covariates(loader = None, use_precomputed_loader=True, precomputed_loader=operative_loader).to_csv(saveName)
...
def stack_matrices_and_bonferonni_correct(results_dir = raw_matrices_save_path_prs, fillwithNA = True, orderbySig = False, include_mb_metab = False):
    ...
    res_corrected = pd.DataFrame(multipletests(pvals = res.to_numpy().flatten(), method = "bonferroni")[1].reshape(res.shape))
...
def prstoGwas(stackmat, threshold= 0.7, do_corr = False, onlyMultiloaders = False):
    ...
    if do_corr:
        clust_col_identity = pd.Series(index=corr.columns, data=clst).iloc[dn['leaves']] ##for each PRS or phenotype, gives which cluster it's in
    else:
    ...
    for identity in range(clust_col_identity.max()):
        if len(clust_col_identity[clust_col_identity.eq(identity)]) > 1: ##only print clusters with size   > 1
            phenos, loaders = clust_col_identity[clust_col_identity.eq(identity)].index.get_level_values(0), clust_col_identity[clust_col_identity.eq(identity)].index.get_level_values(1)
...
def report_pheno(pheno, descriptionmap, stacked):
    print("Phenotype: ", pheno, ", Loader: ", stacked.index.get_level_values(1).values[stacked.index.get_level_values(0) == pheno][0])
...
if __name__ == "__main__":
    ...
    if redo_association_tests_prs:
        ...
        update_covariates(status_table=status_table)  ##We use the 10 PCS from the GWAS and age and gender as covariates in the model, so let's keep them current
