...
def q_loop(test = "t", duplicate_rows = "last",  prs_from_loader = None, saveName = None, tailsTest = "rightLeft", random_shuffle_prsLoader = False, use_prsLoader = True, prs_id = 0): ##test can be "t" for t test or "r" for regression
    ...
    with qp(jobname= saveName[0:2]+str(prs_id), max_u = 100, max_r = 100, _suppress_handlers_warning= True) as q:
        ...
        if test == "t" or test == "m":
            dataprs = preprocess_loader_data( duplicate_rows = duplicate_rows, keep_prs=False, prs_from_loader = prs_from_loader, saveName = saveName, random_shuffle_prsLoader = random_shuffle_prsLoader, use_prsLoader = use_prsLoader)
            dataprs_prs_index = dataprs.set_index("PRS_class", drop=True)
        else:
            dataprs = preprocess_loader_data(duplicate_rows = duplicate_rows, keep_prs=True, prs_from_loader = prs_from_loader, saveName = saveName, random_shuffle_prsLoader = random_shuffle_prsLoader, use_prsLoader = use_prsLoader)
        if dataprs is None:
            return None
