...
def q_generate_prs_matrix(test="m", duplicate_rows="mean", saveName=None, tailsTest="rightLeft",
    ...
    with qp(jobname=saveName, delay_batch=10, _suppress_handlers_warning=True) as q:
        ...
        if use_prsLoader:
            ...
            prses = pd.read_csv(
                "/net/mraid08/export/jasmine/zach/scores/score_results/SOMAscan/scores_all_raw.csv").set_index(
        ...
        for prs_id in range(len(prses)):
            ...
            fundict[prs_id] = q.method(q_loop, (
            test, duplicate_rows, prses[prs_id], saveName, tailsTest, random_shuffle_prsLoader, use_prsLoader,
            prs_id))  ##test can be "t" for t test or "r" for regression))
