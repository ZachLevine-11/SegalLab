...
def excludeBasedOnPersonalMedicalConditions(whattheyhave, medData):
    ...
    if thecode == None:
        ...
        ##We have a match
        return list(medicalConditions.loc[medicalConditions["medical_condition"] == thecode, "RegistrationCode"].unique())
...
def prsToExcluded_people(prsName, h_2_description_dict):
    ...
    if "Non-cancer illness code, self-reported: " in prsmeaning or "Diagnoses - main ICD10" in prsmeaning or "problem" in prsmeaning:
        ...
        if "None" in what:
            ...
            return list(meddata.loc[list(map(lambda medEntry: what in medEntry, meddata["medication_english_only"])), "RegistrationCode"].unique())
...
def get_exclusion_map(use_cached = True, cache  = "/net/mraid08/export/jasmine/zach/prs_associations/exclusionmap_cache.txt"):
    ...
    if use_cached == True:
        ...
        res = {}
        h2_description_dict = PRSLoader().get_data().df_columns_metadata.reset_index().set_index("phenotype_code").h2_description.to_dict()
