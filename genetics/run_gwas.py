...
##Main GWAS Set
loaders_list = [CGMLoader, UltrasoundLoader, ItamarSleepLoader, HormonalStatusLoader, DEXALoader, RetinaScanLoader,
                GutMBLoader, SerumMetabolomicsLoader]
...
def read_loader_in(loader, numeric_cols="notstrict", groupby="latest", sample_size_frac=0.95, remove_sigmas=5,
    ...
    if loader == "Noam":  ##The only time this can take a string
        df = pd.read_csv(
            "/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/biological_age/raw_data_from_noam/residuals_df_equalzscore.csv").set_index(
            "RegistrationCode")
        ...
        ##Ayya's new version
        df = pd.read_csv("/net/mraid08/export/genie/LabData/Analyses/ayyak/CGM/iglu/iglu_no_tails.csv").set_index(
        ...
        df = fix_norm_dist_capping_type_conversion(
            pd.read_csv("/net/mraid08/export/genie/LabData/Analyses/ayyak/CGM/iglu/iglu_no_tails.csv").set_index(
                "id").drop("Unnamed: 0", axis=1),
            cappedDf=NormDistCapping(sample_size_frac=sample_size_frac, remove_sigmas=remove_sigmas).fit_transform(df))
    ...
    if loader == QuestionnairesLoader:  ##cols 734 to 804 in Questionnaires are na for everyone (empty), remove them to avoid future difficulties
        ...
        df = df.reset_index().set_index("RegistrationCode").drop("Date", 1).drop("Start",
                                                                                 1)  ##just consider anyone who took a medication in the past
    else:
        pass
    if loader in [LifeStyleLoader, QuestionnairesLoader,
                  ChildrenLoader]:  ##force some columns to be numeric if we know already that they are, the rest get encoded as dummies
        if numeric_cols == "strict":
            force_numeric_cols_list = [x for x in df.columns if
                                       "weight" in x or "age" in x or "number" in x or "times" in x or "distance" in x or "people_living_together" in x or "number" in x or "how_often" in x or "duration" in x or "minute" in x or "hour" in x or "day" in x or "week" in x or "month" in x or "year" in x]
        else:
            force_numeric_cols_list = [x for x in df.columns if
                                       "weight" in x or "age" in x or "distance" in x or "duration" in x or "minute" in x]
    ...
    for col in df.columns:
        if col[0] == "\n" or len(col.replace("_",
                                             "")) < 2:  ##if we only have one characters in the column name (that wasn't space), get rid of it
...
def make_plink2_command(pheno_name, use_short_names=False, short_names_table_fname=short_name_dict_fname, batched=True,
                        i=0, use_pfilter=True, ldmethod="clump", howmanyPCs=10, do_noam=False):
    if ldmethod == "clump":
    ...
    if not do_noam:
        if batched and use_pfilter:
            cmd = plink2_bin + ' --bfile ' + bfile_loc + ' --king-cutoff 0.22 --pheno /net/mraid08/export/jasmine/zach/height_gwas/all_gwas/phenos_batched/batch' + str(
                i) + '.txt' + ' --1 --mac 20 --linear no-x-sex hide-covar cols=+ax -allow-no-sex --out /net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_results/batch' + str(
                i) + ' --covar /net/mraid08/export/jasmine/zach/height_gwas/covariates_with_age_gender.txt --covar-col-nums 2-' + str(
            ...
            cmd = plink2_bin + ' --bfile ' + bfile_loc + ' --king-cutoff 0.22  --pheno /net/mraid08/export/jasmine/zach/height_gwas/all_gwas/phenos/' + pheno_name + '_pheno.txt' + ' --1 --mac 20 --pheno-name ' + pheno_name + ' --linear no-x-sex hide-covar cols=+ax --allow-no-sex --out /net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_results/' + pheno_name + ' --covar /net/mraid08/export/jasmine/zach/height_gwas/covariates_with_age_gender.txt --covar-col-nums 2-' + str(
                howmanyPCs + 3) + ' --variance-standardize --pfilter 0.00000005'
        else:
            ...
            cmd = plink2_bin + ' --bfile ' + bfile_loc + ' --king-cutoff 0.22  --pheno /net/mraid08/export/jasmine/zach/height_gwas/all_gwas/phenos/' + short_file_name + '_pheno.txt' + ' --1 --mac 20 --pheno-name ' + short_file_name + ' --linear no-x-sex hide-covar cols=+ax --allow-no-sex --out /net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_results/' + short_file_name + ' --covar /net/mraid08/export/jasmine/zach/height_gwas/covariates_with_age_gender.txt --covar-col-nums 2-' + str(
                howmanyPCs + 3) + ' --variance-standardize --pfilter 0.00000005'
        ...
        cmd = plink2_bin + ' --bfile ' + bfile_loc + ' --king-cutoff 0.22 --pheno /net/mraid08/export/jasmine/zach/height_gwas/all_gwas/biological_age/phenos_batched/batch' + str(
            i) + '.txt' + ' --1 --mac 20 --linear no-x-sex hide-covar cols=+ax -allow-no-sex --out /net/mraid08/export/jasmine/zach/height_gwas/all_gwas/biological_age/gwas_results/batch' + str(
            i) + ' --covar /net/mraid08/export/jasmine/zach/height_gwas/covariates_with_age_gender.txt --covar-col-nums 2-' + str(
...
def update_covariates(dir="/net/mraid08/export/jasmine/zach/height_gwas/", status_table=None, keep_fid=False):
    ...
    old_covar_data = pd.read_csv("/net/mraid08/export/genie/10K/genetics/Gencove/covariates/covariates.eigenvec",
                                 sep="\t")
    save_covar_data = pd.merge(old_covar_data, new_covar_data, left_on="IID", right_on="IID", how="inner").drop("#FID",
                                                                                                                axis=1)
    if keep_fid:
        ...
        for loader in loaders_list:
            ...
            for col in df.columns:
                if overwrite or not overwrite and not existsAlready(
                    ...
                    run_plink2(make_plink2_command(col, use_short_names=True, ldmethod=ldmethod, howmanyPCs=howmanyPCs),
                               "gwas",
                               required_memory_gb("/net/mraid08/export/genie/10K/genetics/Gencove/allsamples_qc.bed"))
    else:
        for i in range(num_batches):
            run_plink2(
                make_plink2_command(pheno_name=None, batched=True, i=i, use_pfilter=use_pfilter, ldmethod=ldmethod,
                                    howmanyPCs=howmanyPCs, do_noam=do_noam), "gwas",
                required_memory_gb("/net/mraid08/export/genie/10K/genetics/Gencove/allsamples_qc.bed"), threads=32)
...
def make_unique_plink_bins():
    ...
    write_plink1_bin(thebin_unique, "/net/mraid08/export/jasmine/zach/height_gwas/all_samples_qc_just_unique.bed",
                     "/net/mraid08/export/jasmine/zach/height_gwas/all_samples_qc_just_unique.bim",
                     "/net/mraid08/export/jasmine/zach/height_gwas/all_samples_qc_just_unique.fam")
...
def make_batches(overwrite=False, batch_size=300,
                 gwas_output_dir="/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_results/",
                 single_pheno_dir="/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/phenos/"):
...
def rename_results(gwas_output_dir="/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_results/",
                   gwas_output_renamed_dir="/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_results_renamed/"):
...
def summarize_gwas(onlythesecols=None, forHist=False, threshold=False, use_clumped=True,
                   containing_dir="/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_results/",
...
def genetics_qc(sexcheck=False, ldmethod="clump"):
    ...
    def gwas_first_pass_sexcheck(
            covariates_file="/net/mraid08/export/jasmine/zach/height_gwas/covariates_with_age_gender.txt",
            containing_dir="/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_extra_qc/"):
    ...
    def gwas_qc_first_pass_no_sexcheck(
            containing_dir="/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_extra_qc/"):
...
def get_duplicate_ids(qc_dir="/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_extra_qc/",
...
def clump(qc_dir="/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_extra_qc/",
    ...
    with qp(jobname="clump",
            _mem_def=f'{required_memory_gb("/net/mraid08/export/jasmine/zach/height_gwas/all_gwas/gwas_extra_qc/allsamples_qc_custom.bed")}G',
                    ...
                    if len(gwas.P) != 0:  # ignore gwases with no significant associations
                        ...
                        if len(gwas.loc[gwas.P < (5 * 10 ** (-8)) / numGwases, :]) >= 1:
                            ...
                            gwas.to_csv(gwas_results_dir + "clumpheader" + long_filename, index=False, header=True,
                                        sep="\t")
                            clump_cmd = plink19_bin + ' --bfile ' + qc_dir + 'allsamples_qc_custom --clump ' + gwas_results_dir + "clumpheader" + long_filename + ' --exclude ' + qc_dir + duplicate_id_snps_fname + ' --out ' + clump_dir + \
...
if __name__ == "__main__":
    ...
    if do_GWAS:
        ...
        all_GWAS(overwrite=True, batched=do_batched, num_batches=lenbatches, use_pfilter=use_pfilter, ldmethod=ldmethod,
                 howmanyPCs=howmanyPCs, do_noam=do_noam)
